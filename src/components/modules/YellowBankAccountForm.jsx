...
export default function YellowBankAccountForm({ initial, onSave, onCancel }) {
  const [form, setForm] = useState(
    initial || { bank: "Yellow Bank", account_type: "", bsb: "", account_number: "" }
  );
  const [bsbInfo, setBsbInfo] = useState(
    initial?.bsb_branch_name ? { branch_name: initial.bsb_branch_name } : null
  );
  ...
  const lookupBSB = async (bsb) => {
    ...
    try {
      ...
      if (json.success) {
        ...
        setForm(f => ({
          ...f,
          bank: json.data.institution_name,
          bsb_institution_name: json.data.institution_name,
          bsb_branch_name: json.data.branch_name,
          bsb_address: json.data.address + ", " + json.data.suburb + " " + json.data.state,
        }));
      } else {
        setBsbInfo(null);
    ...
    <div className="space-y-3 p-4 bg-muted/30 rounded-xl border">
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <Input className="mt-1" placeholder="e.g. Savings, Everyday…" value={form.account_type} onChange={e => setForm(f => ({ ...f, account_type: e.target.value }))} />
        </div>
        <div>
