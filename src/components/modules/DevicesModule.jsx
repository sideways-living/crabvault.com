...
import { Plus, Pencil, Trash2, Smartphone, Check, X } from "lucide-react";
...
const EMPTY_FORM = { brand: "", model: "", imei: "", colour: "", used_for: [], notes: "" };
      ...
      <div className="flex items-start justify-between">
        <div className="space-y-1 flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            {device.colour && <span className="text-xs text-muted-foreground">· {device.colour}</span>}
          </div>
          {device.imei && (
            <p className="text-xs font-mono text-muted-foreground pl-5">IMEI: {device.imei}</p>
          )}
          ...
          {device.notes && (
            <p className="text-xs text-muted-foreground pl-5 italic">{device.notes}</p>
          )}
        </div>
